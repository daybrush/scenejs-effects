...
function typing(_a) {
  ...
  if (start < end) {
    for (var i = start; i < end; ++i) {
      item.set.apply(item, [i / length * 100 + "%"].concat(properties, [text.substring(start, i)]));
    }
  } else {
    for (var i = end; i < start; ++i) {
      item.set.apply(item, [i / length * 100 + "%"].concat(properties, [text.substring(end, start + end - i)]));
    }
  }
